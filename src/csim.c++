	...
	for (int ii = 1; ii < nsteps; ii ++) {
		if (sim->h / steps[ii] > 0.0) {
			...
			*v = body->Velocity();
			...
			error("{h} / {steps["+std::to_string(ii)+"]} ("+std::to_string(sim->h)+" / "+std::to_string(steps[ii])+
		...
		for (int ii = 0; ii < sim->nplanets; ii ++) {
			if (sim->read[ii] != NULL) {
				if (sim->read[ii] != body) {
					...
					printrln(in("BulirschStoer", "BSForce")+"    Net force vector on "+body->Name()+" is ", body->net.info(), 4);
				}
			}
		}
    	a = net / body->Mass() * dt;
